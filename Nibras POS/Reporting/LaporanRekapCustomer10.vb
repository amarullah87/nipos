...
Public Class LaporanRekapCustomer10
    ...
    Private Sub LaporanRekapCustomer_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If printerName = "empty" Then
            CRV.SelectionFormula = "{v2_rekap_penjualan_cust10.tgl_jual} IN Date('" & DTPHarian.Value.ToString("yyyy-MM-dd") & "') TO Date('" & DTPBulanan.Value.ToString("yyyy-MM-dd") & "') "
            cryRpt.Load("PenjualanCustomerRekap10.rpt")
            Call seting_laporan()
    ...
    Private Sub btnBatal_Click(sender As Object, e As EventArgs) Handles btnBatal.Click
        ...
        If lookUpEdit.Text.ToString = "" Then
            CRV.SelectionFormula = "{v2_rekap_penjualan_cust10.tgl_jual} IN Date('" & DTPHarian.Value.ToString("yyyy-MM-dd") & "') TO Date('" & DTPBulanan.Value.ToString("yyyy-MM-dd") & "') "
        Else
            CRV.SelectionFormula = "{v2_rekap_penjualan_cust10.tgl_jual} IN Date('" & DTPHarian.Value.ToString("yyyy-MM-dd") & "') TO Date('" & DTPBulanan.Value.ToString("yyyy-MM-dd") & "') AND {v2_rekap_penjualan_cust10.group_member} = '" & lookUpEdit.EditValue.ToString & "' "
        End If
        If printerName = "empty" Then
            cryRpt.Load("PenjualanCustomerRekap10.rpt")
            Call seting_laporan()
