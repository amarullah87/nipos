...
Public Class TransaksiPembelian
    ...
    Private Sub DGV_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles DGV.CellEndEdit
        Dim diskon As Double = DGV.Rows(e.RowIndex).Cells(5).Value / 100
        Dim diskonRupiah As Double = (DGV.Rows(e.RowIndex).Cells(2).Value * DGV.Rows(e.RowIndex).Cells(4).Value) * diskon
            ...
            For barisatas As Integer = 0 To DGV.RowCount - 1
                For barisbawah As Integer = barisatas + 1 To DGV.RowCount - 1
                    If DGV.Rows(barisbawah).Cells(0).Value = DGV.Rows(barisatas).Cells(0).Value Then
                        ...
                        Dim diskonEdit As Double = DGV.Rows(barisatas).Cells(5).Value / 100
                        Dim qtyEdit As Double = DGV.Rows(barisatas).Cells(4).Value + 1
                        Dim diskonRupiahEdit As Double = (DGV.Rows(barisatas).Cells(2).Value * qtyEdit) * diskonEdit

                        DGV.Rows(barisatas).Cells(4).Value = qtyEdit
                        DGV.Rows(barisatas).Cells(6).Value = (DGV.Rows(barisatas).Cells(2).Value * qtyEdit) - diskonRupiahEdit
        ...
        If e.ColumnIndex = 2 Then
            Try
                ...
                DGV.Rows(e.RowIndex).Cells(6).Value = (DGV.Rows(e.RowIndex).Cells(2).Value * DGV.Rows(e.RowIndex).Cells(3).Value) - diskonRupiah
                DGV.CurrentCell = DGV(2, DGV.CurrentCell.RowIndex)
                SendKeys.Send("{up}")
        ...
        If e.ColumnIndex = 3 Then
            Try
                DGV.Rows(e.RowIndex).Cells(4).Value = DGV.Rows(e.RowIndex).Cells(3).Value
                DGV.Rows(e.RowIndex).Cells(6).Value = (DGV.Rows(e.RowIndex).Cells(2).Value * DGV.Rows(e.RowIndex).Cells(3).Value) - diskonRupiah
        ...
        If e.ColumnIndex = 4 Then
            Try
                DGV.Rows(e.RowIndex).Cells(6).Value = (DGV.Rows(e.RowIndex).Cells(2).Value * DGV.Rows(e.RowIndex).Cells(4).Value) - diskonRupiah
            ...
            Catch ex As Exception
                ...
                SendKeys.Send("{up}")
                DGV.Rows(e.RowIndex).Cells(5).Value = DGV.Rows(e.RowIndex).Cells(2).Value * DGV.Rows(e.RowIndex).Cells(4).Value
                Call Hitungtransaksi()
            End Try
        End If
    ...
    Sub Hitungtransaksi()
        ...
        Dim y As Integer = 0
        For baris As Integer = 0 To DGV.RowCount - 1
            y = y + DGV.Rows(baris).Cells(6).Value
        ...
        Dim qtyNis As Integer = 0
        For baris As Integer = 0 To DGV.RowCount - 1
            qtyNis = qtyNis + DGV.Rows(baris).Cells(3).Value
            lbljumlahbarang_nis.Text = qtyNis
        Next
    ...
    Private Sub txtBarcode_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtBarcode.KeyPress
        ...
        If e.KeyChar = ChrW(Keys.Return) Then
            ...
            If (DR.HasRows) Then
                ...
                DGV.Rows.Add(DR.Item("kode_item"), DR.Item("nama_barang"), DR.Item("hpj"), 0, 1, minDisc)
                txtBarcode.Clear()
                cekClose()

                Dim diskon As Double = DGV.Rows(baris).Cells(5).Value / 100
                Dim diskonRupiah As Double = (DGV.Rows(baris).Cells(2).Value * DGV.Rows(baris).Cells(4).Value) * diskon
                DGV.Rows(baris).Cells(6).Value = (DGV.Rows(baris).Cells(2).Value * DGV.Rows(baris).Cells(4).Value) - diskonRupiah
            ...
            Else
                cekClose()
                PencarianBarang.ShowDialog()
                PencarianBarang.TXTCariBarang.Text = txtBarcode.Text
            End If
        End If
    End Sub
    ...
    Private Sub PictureBox1_Click(sender As Object, e As EventArgs) Handles PictureBox1.Click
        ...
        End If
        'MsgBox("Akan rilis pada versi 1.12 Ditunggu yaa....", MsgBoxStyle.Information, "NiPOS")
    End Sub
    ...
    Private Sub SaveTransaksi()
        ...
        For baris As Integer = 0 To DGV.RowCount - 2
            ...
            Dim simpan2 As String = "insert into `detailbeli` (`faktur_beli`,`kode_barang`,`harga_beli`,`qty_beli`,`diskon`,`subtotal_beli`,`ket_beli`,`qty_retur`,`qty_real`,`subtotal_real`) values ('" &
                txtnofaktur.Text & "','" & DGV.Rows(baris).Cells(0).Value & "','" & DGV.Rows(baris).Cells(2).Value & "','" & DGV.Rows(baris).Cells(3).Value & "','" & DGV.Rows(baris).Cells(5).Value & "', '" & DGV.Rows(baris).Cells(6).Value & "', '-', 0, '" & DGV.Rows(baris).Cells(4).Value & "', '" & DGV.Rows(baris).Cells(6).Value & "')"
            ...
            If DR.HasRows Then
                ...
                Dim tambahstok As String = "update barang_m set stok= stok + '" & DGV.Rows(baris).Cells(4).Value & "' where kode_item='" & DGV.Rows(baris).Cells(0).Value & "'"
                Dim stokAwal As Integer = DR.Item("stok")
                Dim stokAkhir As Integer = DGV.Rows(baris).Cells(4).Value + DR.Item("stok")
                cekClose()
                ...
                'Insert History HPP
                Dim diskonEdit As Double = DGV.Rows(baris).Cells(5).Value / 100
                Dim newHPP As Double = DGV.Rows(baris).Cells(2).Value - (DGV.Rows(baris).Cells(2).Value * diskonEdit)
                Dim subtotal As Double = DGV.Rows(baris).Cells(4).Value * newHPP
        ...
        If e.KeyChar = ChrW(Keys.Return) Then
            ...
            If CheckForInternetConnection() Then
                Try
                    ...
                    Dim postbody As String = "kode=" + txtNoPesanan.Text
