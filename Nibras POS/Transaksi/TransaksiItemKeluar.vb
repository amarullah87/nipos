...
Public Class TransaksiItemKeluar
    ...
    Private Sub TransaksiItemKeluar_Load(sender As Object, e As EventArgs) Handles Me.Load
        cekClose()
        SetDoubleBuffered(DGV, True)

        If DaftarItemKeluar.notransaksi = "" Then
            Call Kosongkan()
        Else
            Call GetDetail()
        End If
    ...
    Sub GetDetail()
        Me.Text = "Detail Item Keluar"
        DGV.ReadOnly = True

        cekOpen()
        CMD = New MySqlCommand("SELECT * FROM transaksi_barang WHERE no_transaksi = '" & DaftarItemKeluar.notransaksi & "' ", Conn)
        DR = CMD.ExecuteReader
        DR.Read()
        ...
        cekOpen()
        CMD = New MySqlCommand("SELECT bd.*, b.`nama_barang`, b.`satuan`, b.`jenis` FROM transaksi_barang_detail bd INNER JOIN barang_m b ON b.`kode_item` = bd.`kode_item` WHERE no_transaksi = '" & DaftarItemKeluar.notransaksi & "' ", Conn)
        DR = CMD.ExecuteReader
    ...
    Private Sub txtBarcode_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtBarcode.KeyPress
        ...
        If e.KeyChar = Microsoft.VisualBasic.ChrW(Keys.Return) Then
            ...
            If (DR.HasRows) Then
                ...
                DGV.Rows.Add(DR.Item("kode_item"), DR.Item("nama_barang"), DR.Item("satuan"), DR.Item("jenis"), DR.Item("hpp"), 1)
                DGV.CurrentCell = DGV(5, DGV.CurrentCell.RowIndex)
                txtBarcode.Clear()
                cekClose()
                ...
                Next
            Else
                PencarianBarangItemKeluar.ShowDialog()
                PencarianBarangItemKeluar.TXTCariBarang.Text = txtBarcode.Text
                txtBarcode.Clear()
            End If
        End If
    End Sub
    ...
    Private Sub btnSimpan_Click(sender As Object, e As EventArgs) Handles btnSimpan.Click
        ...
        Else
            cekOpen()
            Dim simpan1 As String = "INSERT INTO transaksi_barang VALUES ('" & txtnofaktur.Text & "','" & Format(dtptanggal.Value, "yyyy-MM-dd") & "','5-2202','" & txtTotalQty.Text & "','" & txtTotal.Text & "', '0', '" & DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") & "', '" & MainMenu.PanelUser.Text & "', '" & txtKeterangan.Text & "' )"
            ...
            ''####### INSERT JURNAL #######''
            cekOpen()
            Dim simpanjurnal1 As String = "insert into jurnal values ('" & txtnofaktur.Text & "','" & Format(dtptanggal.Value, "yyyy-MM-dd") & " " & DateTime.Now.ToString("HH:mm:ss") & "','1-1510', 'PERSEDIAAN BARANG', 0, '" & txtTotal.Text & "', '" & txtKeterangan.Text & "', 3, 1)"
            CMD = New MySqlCommand(simpanjurnal1, Conn)
            CMD.ExecuteNonQuery()

            Dim simpanjurnal2 As String = "insert into jurnal values ('" & txtnofaktur.Text & "','" & Format(dtptanggal.Value, "yyyy-MM-dd") & " " & DateTime.Now.ToString("HH:mm:ss") & "','5-2202', 'ITEM KELUAR','" & txtTotal.Text & "', 0, '" & txtKeterangan.Text & "', 3, 2)"
            ...
            For baris As Integer = 0 To DGV.RowCount - 2
                ...
                If DR.HasRows Then
                    ...
                    Dim tambahstok As String = "update barang_m set stok='" & DR.Item("stok") - DGV.Rows(baris).Cells(5).Value & "' where kode_item='" & DGV.Rows(baris).Cells(0).Value & "'"
                    ...
                    Dim updatehistory As String = "INSERT INTO history_stok VALUES ( '" &
                            ...
                            txtnofaktur.Text & "', '" &
                            Format(dtptanggal.Value, "yyyy-MM-dd") & "', 'Barang Keluar Non-Modal', '" &
                            stokAwal & "', '" &
            ...
            Call InsertLogTrans(txtnofaktur.Text, "CREATE", MainMenu.PanelUser.Text, "ITEM KELUAR | " & txtKeterangan.Text & " QTY " & txtTotalQty.Text & " TOTAL Rp. " & txtTotal.Text)

            MsgBox("Data Berhasil Disimpan")
            Call Kosongkan()
            Call NomorOtomatis()
            DaftarItemKeluar.TampilGrid()
        End If
    End Sub
End Class