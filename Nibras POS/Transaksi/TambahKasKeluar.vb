...
Public Class TambahKasKeluar
    ...
    Sub GenerateTransactionNum()
        cekOpen()
        CMD = New MySqlCommand("select COUNT(no_transaksi) AS no_transaksi from arus_kas WHERE tanggal = '" & Format(dtptanggal.Value, "yyyy-MM-dd") & "' AND no_transaksi LIKE 'KK%' ", Conn)
        DR = CMD.ExecuteReader
        DR.Read()

        If DR.HasRows Then
            Dim inc As Integer = DR.Item("no_transaksi") + 1
            txtNoTransaksi.Text = "KK" + Format(dtptanggal.Value, "yyMMdd") + String.Format("{0:000}", inc)
        Else
    ...
    Sub GenerateTransactionNumX()
        cekOpen()
        CMD = New MySqlCommand("select max(no_transaksi) AS no_transaksi from arus_kas where no_transaksi LIKE 'KK" & Format(Now, "yyMMdd") & "%' ", Conn)
        DR = CMD.ExecuteReader
        DR.Read()

        If IsDBNull(DR.Item("no_transaksi")) Then
            txtNoTransaksi.Text = "KK" + Format(Now, "yyMMdd") + "001"
        Else
            Dim nofaktur As String = DR.Item("no_transaksi")
            Dim inc As Integer = nofaktur.Substring(8, 3) + 1
            txtNoTransaksi.Text = "KK" + Format(Now, "yyMMdd") + String.Format("{0:000}", inc)
        End If
        cekClose()
    End Sub
        ...
        Else
            ...
            If DGV.RowCount > 0 Then
                    ...
                    For baris As Integer = 0 To DGV.RowCount - 1
                        ...
                        CMD = New MySqlCommand("INSERT INTO jurnal VALUES ('" & txtNoTransaksi.Text & "', '" & Format(dtptanggal.Value, "yyyy-MM-dd") & " " & DateTime.Now.ToString("HH:mm:ss") & "', '" &
                                           DGV.Rows(baris).Cells(0).Value & "', '" & DGV.Rows(baris).Cells(1).Value & "', '" & DGV.Rows(baris).Cells(3).Value & "', '0', '" & txtKeterangan.Text & "', '4', '" & baris + 2 & "' )", Conn)
                        CMD.ExecuteNonQuery()
                    ...
                    Call InsertLogTrans(txtNoTransaksi.Text, "CREATE", MainMenu.PanelUser.Text, "KAS KELUAR | " & txtKeterangan.Text & " TOTAL Rp. " & txtJumlah.Text)

                    Call Kosongkan()
                    MsgBox("Data Berhasil Disimpan!", MsgBoxStyle.Information)
                    DaftarKasKeluar.TampilGrid()
                    Me.Close()
                Else
